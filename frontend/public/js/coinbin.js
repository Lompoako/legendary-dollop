$(document).ready(function() {
	...
	var explorer_tx = "http://localhost:22021/api/getchaintxstats" || "https://coinb.in/tx/" 
	var explorer_addr = "http://localhost:22021/api/getaddressinfo/" || "https://coinb.in/addr/"
	var explorer_block = "http://localhost:22021/api/getbestblockhash " ||"https://coinb.in/block/"
	...
	function listUnspentCryptoidinfo_Carboncoin(redeem) {
		...
		$.ajax ({
			type: "POST",
			url: "http://localhost:22021/api/listunspent",
			...
			error: function() {
				$("#redeemFromStatus").removeClass('hidden').html('<span class="glyphicon glyphicon-exclamation-sign"></span> Unexpected error, unable to retrieve unspent outputs!');
                        ...
                        success: function(data) {
				if($(data).find("result").text()==1){
					$("#redeemFromAddress").removeClass('hidden').html('<span class="glyphicon glyphicon-info-sign"></span> Retrieved unspent inputs from address <a href="'+explorer_addr+'" target="_blank">'+redeem.addr+'</a>');
					$.each($(data).find("unspent").children(), function(i,o){
						var tx = $(o).find("tx_hash").text();
						var n = $(o).find("tx_output_n").text();
	...
	function rawSubmitDefault(btn){ 
		...
		$.ajax ({
			...
			url: "http://localhost:22021/api/sendrawtransaction/"+$(data)+'?uid='+coinjs.uid+'&key='+coinjs.key+'&setmodule=bitcoin&request=sendrawtransaction',
			dataType: "xml",
			error: function(data) {
				...
				if($(data).find("result").text()==1){
					...
					//$("#rawTransactionStatus").addClass('alert-success').removeClass('alert-danger').removeClass("hidden").html(' TXID: ' + $(data).find("txid").text() + '<br> <a href="http://localhost:22021/api/sendrawtransaction/'+$(data).find("txid").text() + '" target="_blank">View on Blockchain</a>');
				} else {
